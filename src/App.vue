<template>
  <div id="app">
    <ParticlesBackground />
    <!-- Navigation -->
    <AppNavigation />
    
    <!-- Main Content -->
    <main id="main-content">
      <HeroSection v-scroll-reveal="{ direction: 'fade', duration: 1.2 }" />
      
      <!-- Séparateur entre sections -->
      <AnimatedSectionDivider type="gradient" height="medium" />
      
      <AboutSection v-scroll-reveal="{ direction: 'up', distance: 60, duration: 1 }" />
      
      <!-- Séparateur entre sections -->
      <AnimatedSectionDivider type="gradient" height="medium" />
      
      <ExperienceSection v-scroll-reveal="{ direction: 'up', distance: 80, duration: 1.2, delay: 0.2 }" />
    </main>
    
    <!-- Footer -->
    <footer class="footer">
      <div class="footer__container">
        <div class="footer__content">
          <div class="footer__info">
            <h3 class="footer__name">{{ personalInfo.name }}</h3>
            <p class="footer__title">{{ personalInfo.title }}</p>
            <p class="footer__location">{{ personalInfo.location }}</p>
          </div>
          
          <div class="footer__links">
            <a 
              v-for="social in personalInfo.socialLinks" 
              :key="social.name"
              :href="social.url" 
              target="_blank" 
              rel="noopener noreferrer"
              class="footer__social-link"
              :title="social.name"
              v-accessible
              v-micro-interaction="{ type: 'hover', scale: 1.1 }"
              v-cursor-interactive="{ type: 'pointer' }"
            >
              <div v-html="getSocialIcon(social.icon)"></div>
            </a>
          </div>
        </div>
        
        <div class="footer__bottom">
          <p class="footer__copyright">
            © {{ currentYear }} {{ personalInfo.name }}. All rights reserved.
          </p>
          <p class="footer__built">
            Built with Vue.js & TypeScript
          </p>
        </div>
      </div>
    </footer>
  </div>
</template>

<script setup lang="ts">
import { computed, onMounted } from 'vue'
import ParticlesBackground from '@/components/effects/ParticlesBackground.vue'
import AppNavigation from '@/components/layout/AppNavigation.vue'
import HeroSection from '@/components/sections/HeroSection.vue'
import AboutSection from '@/components/sections/AboutSection.vue'
import ExperienceSection from '@/components/sections/ExperienceSection.vue'
import AnimatedSectionDivider from '@/components/effects/AnimatedSectionDivider.vue'

import { personalInfo } from '@/data/portfolio'

// Import des composables pour les améliorations
import { useAccessibility, vAccessible } from '@/composables/useAccessibility'
import { useLoadingAnimations } from '@/composables/useLoadingAnimations'
import { useMicroInteractions, vMicroInteraction } from '@/composables/useMicroInteractions'
import { useCustomCursor, vCursorInteractive } from '@/composables/useCustomCursor'
import { useScrollAnimations, vScrollReveal, vParallax } from '@/composables/useScrollAnimations'
import { usePerformanceOptimization } from '@/composables/usePerformanceOptimization'

// Initialisation des composables
const { enhanceFocusVisibility, setupKeyboardNavigation } = useAccessibility()
const { startLoading, finishLoading } = useLoadingAnimations()
const { } = useMicroInteractions()
const { initCursor } = useCustomCursor()
const { initScrollAnimations } = useScrollAnimations()
const { initPerformanceOptimizations, animationQuality } = usePerformanceOptimization()

// Initialisation au montage du composant
onMounted(async () => {
  // Initialiser les optimisations de performance en premier
  initPerformanceOptimizations()
  
  // Démarrer l'animation de chargement
  startLoading()
  
  // Initialiser les améliorations d'accessibilité
  enhanceFocusVisibility()
  setupKeyboardNavigation(document.body)
  
  // Initialiser le curseur personnalisé et les animations de scroll
  initCursor()
  initScrollAnimations()
  
  // Simuler un temps de chargement adapté à la performance de l'appareil
  const loadingDuration = animationQuality.value === 'low' ? 800 : animationQuality.value === 'medium' ? 1200 : 1500
  setTimeout(() => {
    finishLoading()
  }, loadingDuration)
})

// Année courante pour le copyright
const currentYear = computed(() => new Date().getFullYear())

// Icônes des réseaux sociaux
const getSocialIcon = (iconName: string): string => {
  const icons: Record<string, string> = {
    github: '<svg viewBox="0 0 24 24" fill="currentColor"><path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z"/></svg>',
    linkedin: '<svg viewBox="0 0 24 24" fill="currentColor"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>',
    twitter: '<svg viewBox="0 0 24 24" fill="currentColor"><path d="M23.953 4.57a10 10 0 01-2.825.775 4.958 4.958 0 002.163-2.723c-.951.555-2.005.959-3.127 1.184a4.92 4.92 0 00-8.384 4.482C7.69 8.095 4.067 6.13 1.64 3.162a4.822 4.822 0 00-.666 2.475c0 1.71.87 3.213 2.188 4.096a4.904 4.904 0 01-2.228-.616v.06a4.923 4.923 0 003.946 4.827 4.996 4.996 0 01-2.212.085 4.936 4.936 0 004.604 3.417 9.867 9.867 0 01-6.102 2.105c-.39 0-.779-.023-1.17-.067a13.995 13.995 0 007.557 2.209c9.053 0 13.998-7.496 13.998-13.985 0-.21 0-.42-.015-.63A9.935 9.935 0 0024 4.59z"/></svg>'
  }
  
  return icons[iconName] || '<svg viewBox="0 0 24 24" fill="currentColor"><circle cx="12" cy="12" r="10"/></svg>'
}
</script>

<style>
/* Import des styles globaux */
@import '@/assets/styles/main.css';

#app {
  min-height: 100vh;
  display: flex;
  flex-direction: column;
}

main {
  flex: 1;
}

/* Footer Styles */
.footer {
  background: var(--color-surface);
  border-top: 1px solid var(--color-border);
  padding: 3rem 0 1.5rem;
  margin-top: auto;
}

.footer__container {
  max-width: 1200px;
  margin: 0 auto;
  padding: 0 2rem;
}

.footer__content {
  display: flex;
  justify-content: space-between;
  align-items: center;
  margin-bottom: 2rem;
  padding-bottom: 2rem;
  border-bottom: 1px solid var(--color-border);
}

.footer__info {
  flex: 1;
}

.footer__name {
  font-size: 1.25rem;
  font-weight: 700;
  color: var(--color-text-primary);
  margin: 0 0 0.5rem 0;
}

.footer__title {
  color: var(--color-primary);
  font-weight: 500;
  margin: 0 0 0.25rem 0;
}

.footer__location {
  color: var(--color-text-secondary);
  font-size: 0.875rem;
  margin: 0;
}

.footer__links {
  display: flex;
  gap: 1rem;
}

.footer__social-link {
  width: 2.5rem;
  height: 2.5rem;
  background: var(--color-background);
  border: 1px solid var(--color-border);
  border-radius: 50%;
  display: flex;
  align-items: center;
  justify-content: center;
  color: var(--color-text-secondary);
  text-decoration: none;
  transition: all 0.3s ease;
}

.footer__social-link:hover {
  background: var(--color-primary);
  border-color: var(--color-primary);
  color: white;
  transform: translateY(-2px);
}

.footer__social-link svg {
  width: 1rem;
  height: 1rem;
}

.footer__bottom {
  display: flex;
  justify-content: space-between;
  align-items: center;
  font-size: 0.875rem;
  color: var(--color-text-secondary);
}

.footer__copyright,
.footer__built {
  margin: 0;
}

@media (max-width: 768px) {
  .footer__content {
    flex-direction: column;
    gap: 1.5rem;
    text-align: center;
  }
  
  .footer__bottom {
    flex-direction: column;
    gap: 0.5rem;
    text-align: center;
  }
}
</style>
